...
export function exitValue(i: Inputs, out: Outputs): YearExit[] {
  ...
  return [1, 2, 3].map((y) => {
    ...
    const tier = [...EARNINGS_TIERS].reverse().find((t) => adjEbitda >= t.floor)!;
...
export function levers(i: Inputs, y: 1 | 2 | 3): { label: string; delta: number }[] {
  ...
  const moves: [string, InputKey, number][] = [
    ...
    ['FE calls $1 cheaper', 'feCallCost', Math.max(0, i.feCallCost - 1)],
    ['Medicare calls $1 cheaper', 'mdCallCost', Math.max(0, i.mdCallCost - 1)],

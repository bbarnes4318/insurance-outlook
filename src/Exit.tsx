...
function LensTile({ title, sub, win, lose, children }: { title: string; sub: string; win: boolean; lose: string; children: ReactNode }) {
  return (
    <div className={`flex flex-col rounded-xl px-4 py-3 ring-1 transition-opacity ${win ? 'bg-surface2/70 ring-[#f5c451]/50' : 'bg-surface2/30 opacity-50 ring-line/70'}`}>
...
export function Exit({ inputs, out, ex, names, year, setYear }: {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 gap-4 p-4">
      <div className="pointer-events-none absolute left-[20%] top-[-220px] h-[500px] w-[760px] rounded-full opacity-50 blur-[120px]" style={{ background: 'radial-gradient(closest-side, #f5c45133, #199e7026, transparent)' }} />
      ...
      <div className="relative flex w-[440px] shrink-0 flex-col gap-3">
        <div className="flex flex-col gap-2">
          {ex.map((x) => (
            <button key={x.y} onClick={() => setYear(x.y as 1 | 2 | 3)} aria-pressed={x.y === year}
              className={`flex h-[46px] items-center gap-3 rounded-xl px-4 text-left ring-1 transition-colors ${x.y === year ? 'bg-surface2 ring-2 ring-[#f5c451]/80' : 'bg-surface ring-line/70 hover:bg-surface2/60'}`}>
        ...
        <Card className="relative overflow-hidden px-5 pb-4 pt-4">
          ...
          <div className="relative mt-2 h-2 rounded-full" style={{ background: `linear-gradient(90deg, ${GOLD}44, ${GOLD}, ${C.net})` }}>
            <div className="absolute -top-1 h-4 w-1 -translate-x-1/2 rounded-full bg-ink shadow transition-all duration-500" style={{ left: `${at * 100}%` }} />
          </div>
        ...
        <Card className="flex min-h-0 flex-1 flex-col px-5 py-4">
          ...
          <div className="flex h-3 gap-[2px] overflow-hidden rounded-full">
            ...
            <div className="flex-1" style={{ background: '#f5c45133' }} />
          </div>
          ...
          <div className="mt-2 rounded-lg bg-net/10 px-3 py-1.5 text-[12px] text-sub ring-1 ring-net/25">
            <span className="font-semibold text-net">You also keep:</span> {money(e.receivable)} FE commissions still owed to you
          </div>
      ...
      <div className="relative flex min-w-0 flex-1 flex-col gap-3">
          ...
          <div className="flex-1 rounded-xl bg-surface px-4 py-2.5 text-muted ring-1 ring-line/70">
            ...
            {top.map((l) => (
              <div key={l.label} className="flex justify-between gap-2">
                <span className="truncate">{l.label}</span><span className="tnum shrink-0 font-semibold text-net">{l.delta < 0 ? '' : '+'}{compact(l.delta)}</span>
              </div>
            ))}
          </div>
          {e.lockedIn > 0 && (
            <div className="flex-1 rounded-xl bg-[#f5c451]/10 px-4 py-2.5 text-muted ring-1 ring-[#f5c451]/25">
        ...
        <div className="-mt-1 shrink-0 text-[11px] text-muted">
          Ranges reflect FE/Medicare agency deal multiples. Price is pre-tax unless tax on sale is set. Earnout paid over 24 months subject to retention.
        </div>
      </div>
    </div>
  );
